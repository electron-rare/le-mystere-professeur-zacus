...
#include "../include/ui_screen_loader.h"
...
void setup() {
  ...
  if (!LittleFS.begin()) {
    Serial.println("[UI_TFT] LittleFS mount FAIL");
  } else {
    Serial.println("[UI_TFT] LittleFS mount OK");
    g_nav.loadScreen("SCENE_LOCKED");
    UiRenderer::showLocalScreen(g_tft, "/SCENE_LOCKED.json");
    delay(1000);
  }
...
void loop() {
  ...
  // Affichage de l'écran courant (simple)
  UiRenderer::showLocalScreen(g_tft, ("/" + g_nav.currentScreen().id + ".json").c_str());
