...
constexpr uint32_t kUiDiagBootWindowMs = 20000u;
constexpr uint16_t kUiDiagLogLimit = 48u;
...
bool UiLink::diagEnabled(uint32_t nowMs) const {
  if (diagStartMs_ == 0u) {
    return false;
  }
  return static_cast<uint32_t>(nowMs - diagStartMs_) <= kUiDiagBootWindowMs;
}

void UiLink::diagLogTx(const char* type, size_t lineLen, uint32_t nowMs) {
  if (!diagEnabled(nowMs) || diagTxLogCount_ >= kUiDiagLogLimit) {
    return;
  }
  ++diagTxLogCount_;
  Serial.printf("[UI_DIAG][ESP32][TX] ms=%lu type=%s len=%u tx=%lu drop=%lu\n",
                static_cast<unsigned long>(nowMs),
                (type != nullptr) ? type : "?",
                static_cast<unsigned int>(lineLen),
                static_cast<unsigned long>(txFrameCount_),
                static_cast<unsigned long>(txDropCount_));
}

void UiLink::diagLogRx(const UiLinkFrame& frame, uint32_t nowMs) {
  if (!diagEnabled(nowMs) || diagRxLogCount_ >= kUiDiagLogLimit) {
    return;
  }
  ++diagRxLogCount_;
  Serial.printf("[UI_DIAG][ESP32][RX] ms=%lu type=%s fields=%u crc=%u rx=%lu\n",
                static_cast<unsigned long>(nowMs),
                frame.type_token,
                static_cast<unsigned int>(frame.field_count),
                frame.crc_ok ? 1u : 0u,
                static_cast<unsigned long>(rxFrameCount_));
}

void UiLink::diagLogError(const char* reason, const char* line, uint32_t nowMs) {
  if (!diagEnabled(nowMs) || diagErrLogCount_ >= kUiDiagLogLimit) {
    return;
  }
  ++diagErrLogCount_;
  const char* safeReason = (reason != nullptr) ? reason : "unknown";
  const char* safeLine = (line != nullptr) ? line : "";
  Serial.printf("[UI_DIAG][ESP32][ERR] ms=%lu reason=%s line=%s parse=%lu crc=%lu\n",
                static_cast<unsigned long>(nowMs),
                safeReason,
                safeLine,
                static_cast<unsigned long>(parseErrorCount_),
                static_cast<unsigned long>(crcErrorCount_));
}
...
void UiLink::begin() {
  ...
  diagRxLogCount_ = 0u;
  diagErrLogCount_ = 0u;
  Serial.printf("[UI_DIAG][ESP32] begin baud=%lu rx_pin=%u tx_pin=%u window_ms=%lu\n",
                static_cast<unsigned long>(baud_),
                static_cast<unsigned int>(rxPin_),
                static_cast<unsigned int>(txPin_),
  ...
  switch (frame.type) {
    case UILINK_MSG_HELLO: {
      ...
      if (proto == nullptr || strcmp(proto->value, "2") != 0) {
        diagLogError("hello_proto", frame.type_token, nowMs);
        return false;
      }
      connected_ = true;
      lastRxMs_ = nowMs;
      ...
      Serial.printf("[UI_DIAG][ESP32][HELLO] ms=%lu session=%lu ack_pending=1\n",
                    static_cast<unsigned long>(nowMs),
    ...
    case UILINK_MSG_PONG:
      ...
      lastRxMs_ = nowMs;
      ++pongRxCount_;
      if (diagEnabled(nowMs)) {
        Serial.printf("[UI_DIAG][ESP32][PONG] ms=%lu pong=%lu\n",
                      static_cast<unsigned long>(nowMs),
                      static_cast<unsigned long>(pongRxCount_));
      }
      return true;
    case UILINK_MSG_BTN: {
      connected_ = true;
...
bool UiLink::sendAck() {
  ...
  lastTxMs_ = millis();
  ++txFrameCount_;
  diagLogTx("ACK", lineLen, lastTxMs_);
  return true;
}
...
bool UiLink::sendStateFrame(const ScreenFrame& frame, bool keyframe) {
  ...
  ++txFrameCount_;
  diagLogTx(type, lineLen, frame.nowMs);
  return true;
}
...
void UiLink::poll(uint32_t nowMs) {
    ...
    if (c == '\n') {
      if (!dropCurrentLine_ && rxLineLen_ > 0u) {
        ...
        if (uiLinkParseLine(rxLine_, &frame)) {
          diagLogRx(frame, nowMs);
          if (!handleIncomingFrame(frame, nowMs)) {
            ++parseErrorCount_;
            diagLogError("frame_handler", rxLine_, nowMs);
          }
        } else {
          if (strchr(rxLine_, '*') != nullptr) {
            ++crcErrorCount_;
            diagLogError("crc", rxLine_, nowMs);
          } else {
            ++parseErrorCount_;
            diagLogError("parse", rxLine_, nowMs);
          }
        }
      }
      rxLineLen_ = 0u;
  ...
  if (connected_ && timeoutMs_ > 0u && static_cast<uint32_t>(nowMs - lastRxMs_) > timeoutMs_) {
    if (diagEnabled(nowMs)) {
      Serial.printf("[UI_DIAG][ESP32][TIMEOUT] ms=%lu last_rx_age=%lu\n",
                    static_cast<unsigned long>(nowMs),
                    static_cast<unsigned long>(nowMs - lastRxMs_));
    }
    connected_ = false;
  }
}
