...
const LiveOrchestrator = ({ scenarioId, onSkip, onPause, onResume, onBack }: LiveOrchestratorProps) => {
  ...
  const handleMessage = useMemo(
    () => (message: StreamMessage) => {
      if (message.type === 'step_change') {
        const step = message.data?.current_step as string | undefined
        const pct = Number(message.data?.progress_pct ?? 0)
        setCurrentStep(step ?? 'Unknown step')
        setProgress(Number.isFinite(pct) ? pct : 0)
        setRunStatus('running')
      }

      if (message.type === 'status' && message.data?.status === 'paused') {
        setRunStatus('paused')
      }
  ...
  const { status } = useStoryStream(getWsUrl(), { onMessage: handleMessage })
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.4fr_1fr]">
        <div className="glass-panel flex flex-col gap-4 rounded-3xl p-6">
          ...
          <div>
            ...
            <div className="mt-2 h-3 rounded-full bg-white/70">
              <div
                className="h-3 rounded-full bg-[var(--teal-500)]"
        ...
        <div className="glass-panel flex flex-col gap-4 rounded-3xl p-6">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              onClick={runStatus === 'paused' ? handleResume : handlePause}
              className="focus-ring min-h-[44px] flex-1 rounded-full bg-[var(--ink-700)] px-4 text-sm font-semibold text-white"
            >
            ...
            <button
              type="button"
              onClick={onSkip}
              className="focus-ring min-h-[44px] flex-1 rounded-full border border-[var(--ink-700)] px-4 text-sm font-semibold text-[var(--ink-700)]"
            >
              Skip
            </button>
            <button
          ...
          <div className="rounded-2xl bg-white/60 p-3 text-xs text-[var(--ink-500)]">
            Shortcuts: pause/resume, skip to next step, or return to selector.
          </div>
        </div>
        ...
        <div className="glass-panel flex flex-col rounded-3xl p-6">
          ...
          <div ref={logRef} className="mt-4 max-h-[320px] space-y-3 overflow-y-auto pr-2 text-xs">
            {events.length === 0 && (
              <p className="text-[var(--ink-500)]">Waiting for events...</p>
            )}
            {events.map((event) => (
