...
| plan | Execute an agent plan by running its `## Plan d’action` commands | tools/dev/cockpit.sh | plan<br><agent><br>[--dry-run]<br>[--plan-only] | .github/agents/core/agent-briefings.md#plan-execution-helper |  |
