...
| **P1** ✅ **livré 2026-05-03** | Install F5-TTS on MacStudio (`pip install f5-tts` or `mlx_audio` if MLX port available), download base checkpoint (~1–2 GB), smoke-test on a fixed FR phrase against a generic reference clip. Smoke-test STT (10-clip FR set) and LLM (hints engine response within ±10% latency of KXKM-AI baseline). Modifs livrées : **F5_DEFAULT_STEPS=4** (au lieu de 8), **F5_TIMEOUT_S=8.0** (au lieu de 3.0), **whisper Q5_0** (au lieu de Q5_K). Daemons MLX livrés : `npc-fast` et `hints-deep` exposés via voice-bridge (au lieu d'Ollama HTTP, voir §2.3). **P1 part9 (à livrer)** : déployer `~/voice-bridge/watchdog.sh` + ligne crontab `*/2 * * * *`, créer `~/voice-bridge/cache/`, pré-rendre `service_down.wav` (« Le Professeur Zacus est temporairement indisponible, un instant »), router `/tts/service_down`. | F5-TTS produces audible FR audio for the smoke phrase ; STT transcript stable ; LLM latency within budget ; voice-bridge `/tts` returns audio with `tts_backend_used=f5` ; watchdog cron actif ; `service_down.wav` jouable | 2-3 h |
...
- [ ] **F5 watchdog test** : `kill $(pgrep -f voice-bridge)` → relance auto < 2 min via crontab `*/2 *` → **PENDING**
- [ ] **Pool pre-render couvre ≥ 80 %** des phrases de `npc_phrases.yaml` (cache `~/voice-bridge/cache/` peuplé) → **PENDING**
- [ ] **`service_down.wav` jouable** + accepté narrativement (« Le Professeur Zacus est temporairement indisponible, un instant ») → **PENDING**
...
- [ ] **Ratio cache hits ≥ 80 %** + **ratio F5 timeouts < 5 %** sur fenêtre soak 24 h (reformulation de l'ancien « Piper fallback ratio < 5 % »)
