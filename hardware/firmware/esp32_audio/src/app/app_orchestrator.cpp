...
void applyStoryV2ActionHook(const StoryActionDef& action, uint32_t nowMs, const char* source) {
  ...
  switch (action.type) {
    case StoryActionType::kTrace:
      Serial.printf("[STORY_V2] action trace id=%s via=%s\n",
    ...
    case StoryActionType::kRequestSdRefresh:
      ...
      Serial.println("[STORY_V2] action refresh SD requested.");
      break;
...
void resetStoryTimeline(const char* source) {
  ...
  if (isStoryV2Enabled()) {
    storyV2Controller().reset(nowMs, source);
    return;
  }
...
void updateStoryTimeline(uint32_t nowMs) {
  if (isStoryV2Enabled()) {
    storyV2Controller().update(nowMs);
    return;
  }
