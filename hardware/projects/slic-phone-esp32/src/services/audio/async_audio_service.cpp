...
bool AsyncAudioService::startFs(fs::FS& storage,
  ...
  fsFile_ = std::unique_ptr<AudioFileSourceFS>(new AudioFileSourceFS(storage, path));
  fsOutput_ = std::unique_ptr<AudioOutputI2S>(new AudioOutputI2S(static_cast<int>(i2sPort_), AudioOutputI2S::EXTERNAL_I2S));
  fsOutput_->SetPinout(static_cast<int>(i2sBclk_),
                       static_cast<int>(i2sLrc_),
                       static_cast<int>(i2sDout_));
  fsOutput_->SetGain(gain);

  AudioGenerator* decoder = createDecoder(codec);
  if (decoder == nullptr) {
    cleanupFs();
    return false;
  }
  fsDecoder_ = std::unique_ptr<AudioGenerator>(decoder);

  if (!fsDecoder_->begin(fsFile_.get(), fsOutput_.get())) {
    cleanupFs();
    return false;
  }
