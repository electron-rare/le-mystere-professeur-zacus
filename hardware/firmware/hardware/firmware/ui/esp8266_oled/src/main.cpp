...
constexpr uint32_t kUiDiagBootWindowMs = 20000;
constexpr uint16_t kUiDiagLogLimit = 64;
...
uint32_t g_uiDiagStartMs = 0;
uint32_t g_uiDiagTxFrames = 0;
uint32_t g_uiDiagRxFrames = 0;
uint32_t g_uiDiagHelloTx = 0;
uint32_t g_uiDiagPongTx = 0;
uint32_t g_uiDiagPingRx = 0;
uint32_t g_uiDiagAckRx = 0;
uint16_t g_uiDiagTxLogs = 0;
uint16_t g_uiDiagRxLogs = 0;
uint16_t g_uiDiagErrLogs = 0;
...
bool uiDiagActive(uint32_t nowMs) {
  if (g_uiDiagStartMs == 0U) {
    return false;
  }
  return (nowMs - g_uiDiagStartMs) <= kUiDiagBootWindowMs;
}

void uiDiagLogTx(const char* type, size_t lineLen) {
  ++g_uiDiagTxFrames;
  ...
  if (!uiDiagActive(nowMs) || g_uiDiagTxLogs >= kUiDiagLogLimit) {
    return;
  }
  ++g_uiDiagTxLogs;
  Serial.printf("[UI_DIAG][ESP8266][TX] ms=%lu type=%s len=%u tx_frames=%lu\n",
                static_cast<unsigned long>(nowMs),
                type,
                static_cast<unsigned int>(lineLen),
                static_cast<unsigned long>(g_uiDiagTxFrames));
}

void uiDiagLogRx(const UiLinkFrame& frame) {
  ++g_uiDiagRxFrames;
  ...
  const uint32_t nowMs = millis();
  if (!uiDiagActive(nowMs) || g_uiDiagRxLogs >= kUiDiagLogLimit) {
    return;
  }
  ++g_uiDiagRxLogs;
  Serial.printf("[UI_DIAG][ESP8266][RX] ms=%lu type=%s fields=%u crc=%u rx_frames=%lu\n",
                static_cast<unsigned long>(nowMs),
                frame.type_token,
                static_cast<unsigned int>(frame.field_count),
                frame.crc_ok ? 1U : 0U,
                static_cast<unsigned long>(g_uiDiagRxFrames));
}

void uiDiagLogError(const char* reason) {
  const uint32_t nowMs = millis();
  if (!uiDiagActive(nowMs) || g_uiDiagErrLogs >= kUiDiagLogLimit) {
    return;
  }
  ++g_uiDiagErrLogs;
  Serial.printf("[UI_DIAG][ESP8266][ERR] ms=%lu reason=%s parse=%lu crc=%lu ovf=%lu\n",
                static_cast<unsigned long>(nowMs),
                reason,
...
void handleIncoming() {
  ...
  if (g_link.overflow()) {
    ++g_rxOverflowCount;
    uiDiagLogError("serial_overflow");
  }
}
...
void setup() {
    LittleFS.begin();
    g_nav.loadScreen("SCENE_LOCKED");
  Serial.begin(115200);
  g_uiDiagStartMs = millis();
  Serial.printf("[UI_DIAG][ESP8266] begin rx_pin=%u tx_pin=%u baud=%lu window_ms=%lu\n",
                static_cast<unsigned int>(kLinkRx),
                static_cast<unsigned int>(kLinkTx),
                static_cast<unsigned long>(kLinkBaud),
                static_cast<unsigned long>(kUiDiagBootWindowMs));
  initDisplay();
  if (g_linkState.linkEnabled) {
...
void loop() {
    g_nav.update();
    // Affichage OLED de l'écran courant (id + description)
    g_display.clearDisplay();
    g_display.setTextColor(SSD1306_WHITE);
    g_display.setTextSize(1);
    g_display.setCursor(0, 0);
    g_display.print(g_nav.currentScreen().id.c_str());
    g_display.setCursor(0, 16);
    g_display.print(g_nav.currentScreen().description.c_str());
    g_display.display();
    // TODO: brancher ici la gestion des boutons physiques pour appeler g_nav.handleEvent("button_ok"), etc.
  const uint32_t nowMs = millis();
  if (g_linkState.linkEnabled) {
  ...
  if ((nowMs - g_lastDiagMs) >= kDiagPeriodMs) {
    ...
    Serial.printf(
        "[UI_DIAG][ESP8266][SUM] tx_frames=%lu rx_frames=%lu hello_tx=%lu pong_tx=%lu ping_rx=%lu ack_rx=%lu parse=%lu crc=%lu ovf=%lu\n",
        static_cast<unsigned long>(g_uiDiagTxFrames),
        static_cast<unsigned long>(g_uiDiagRxFrames),
        static_cast<unsigned long>(g_uiDiagHelloTx),
        static_cast<unsigned long>(g_uiDiagPongTx),
        static_cast<unsigned long>(g_uiDiagPingRx),
        static_cast<unsigned long>(g_uiDiagAckRx),
        static_cast<unsigned long>(g_parseErrorCount),
        static_cast<unsigned long>(g_crcErrorCount),
        static_cast<unsigned long>(g_rxOverflowCount));
